...
if [ `grep -wc "enable syslog" $syslog` -gt 0 ]
	...
	for i in `grep "host" $syslog | awk '{print $4}'`
		...
		if [ "$syslog_ip" = "192.168.1.120" ] && [ "$syslog_facility" = "local1" ]
			then
			lbd_trap="both"
		fi
...
temp=`grep "config" $impb | grep "ip_mac" | grep "state enable" | awk -Fports '{print $2}' | awk '{print $1}'`
dummy=''
...
temp=`grep "config" $impb | grep "ip_mac" | grep "state enable loose" | awk -Fports '{print $2}' | awk '{print $1}'`
dummy=''
...
for i in $@
	...
	if [ `echo $profile_type | grep -c "type"` -eq 0 ]
		then
		mask=`grep "create" $acl | grep -w "profile_id $i" | awk -F"$profile_type" '{print $2}' | awk -Fprofile_id '{print $1}'`
	...
	for j in $access_id_condition_list
		...
		if [ "$j" = "40" ]
			then
			continue
			# Dirty hack for pkt_content offset on DES-3028 only
		fi
...
for i in `grep "create mcast_filter_profile" $ism | awk -Fprofile_id '{print $2}' | awk '{print $1}'`
	...
	ports=`grep "config limited_multicast_addr" $ism | grep "add profile_id $i" | awk -Fports '{print $2}' | awk '{print $1}'`
	echo "mcast_range.$i.ports=$ports" >> $dry_mcast
done

/usr/local/sbin/collapse_mcast.sh $dry_mcast
igmp_aa_enabled=`grep "access_authentication" $ism | grep "enable" | awk -Fport '{print $2}' | awk '{print $1}'`
igmp_aa_disabled=`grep "access_authentication" $ism | grep "disable" | awk -Fport '{print $2}' | awk '{print $1}'`
